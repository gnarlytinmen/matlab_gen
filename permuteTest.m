...
% Run monte carlo for bootstrapped comparison
bootVals = nan(numReps,1);
for i = 1:numReps
    if compFxn ~= 3
        g1Inds = randi(numel(allPts),groupSize,1);
        g2Inds = randi(numel(allPts),groupSize,1);
        
        bootVals(i,1) = fxn(allPts(g1Inds),allPts(g2Inds));
    else
        g1Inds = randi(groupSize,groupSize,1);
        g2Inds = randi(groupSize,groupSize,1);
        
        grp1 = group1(nonNanPairs);
        grp2 = group2(nonNanPairs);
        
        bootVals(i,1) = fxn(grp1(g1Inds),grp2(g2Inds));
    end
end

% Get probability of drawing difference (two-tailed test)
% p = 2*min((x<X|H_0),x>X(|H_0))
p = 2*min([sum(testVal>bootVals)/numel(bootVals);...
           sum(testVal<bootVals)/numel(bootVals)]);
...
if plotOn
    ...
    subplot(1,2,2);
    hold on;
    histogram(bootVals,100,'FaceColor','k');
    ax2 = gca;
    ymax = ax2.YLim(2);
    xmax = max(ax2.XLim);
    ax2.XLim = [-xmax xmax];
