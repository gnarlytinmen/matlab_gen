function [dots,delta_dots,h]=rand_flowfield2(az,el,trans_vel,purs_angle,rot_vel)
...
dist=50;     % Set screen distance to match setup
...
res=1;    % Set resolution, must be some multiple of (2)^-1 (works best around 0.125-0.25)
h=120;
v=68;
d=100;
...
for i=1:num_dots
    x=res*(dots(i,1)-((vmax)/2));  % Scale and zero-center
    y=res*(dots(i,2)-((hmax)/2));
...
dz_dt=zeros(a,b,c); % Set dummy var for quiver3

[X,Y,Z]=meshgrid(y,x,z);   % make sure -y in bottom portion of graph

figure;clf;hold on;
quiver3(X,Y,Z,dx_dt,dy_dt,dz_dt,8,'Color',[0.3 0.3 0.3]);%,'ShowArrowHead','off');
scatter3(0,0,dist,20,'r','filled');
set(gca,'Visible','off','Box','off','Projection','orthographic','CameraPosition',[0 0 -170]);
%set(gca,'Visible','off','Box','off','Projection','perspective');
set(gca,'YLim',[-a/2 a/2],'XLim',[-b/2 b/2],'ZLim',[0+dist d+dist]);
